...
const StoryBar = () => {
  ...
  const fetchStories = async () => {
    ...
    try {
      ...
      console.log("result stories: ", result);
      if (result.success) {
        console.log("Get Thành Công Stories: ", result.stories);
    ...
    const handleNewStory = ({ story }) => {
      ...
      setStories((prevStories) => {
        ...
        if (existingGroupIndex !== -1) {
          ...
          if (!group.stories.some((s) => s._id.toString() === story._id.toString())) {
        ...
        {stories.map((story, index) => {
          ...
          return (
            <div
              ...
              style={{
                backgroundColor: latestStoryItem?.story_type === "text" ? latestStoryItem?.background_color : undefined,
              }}
            >
              <img
              ...
              <p
                ...
                style={{
                  color: latestStoryItem?.story_type === "text" ? latestStoryItem?.text_color : "rgba(255,255,255,0.85)",
                }}
              >
                {latestStoryItem?.content}
              </p>
              <p className="absolute text-white bottom-1 right-2 z-10 text-[10px] opacity-80">
                {moment(story.latestStory || latestStoryItem?.createdAt).fromNow()}
              </p>
              {latestStoryItem?.story_type !== "text" && latestStoryItem?.media && (
                <div className="absolute inset-0 z-1 rounded-lg bg-black overflow-hidden">
                  {latestStoryItem.media.type === "image" ? (
                    <img
                      src={latestStoryItem.media.url.replace(/^http:\/\//i, "https://")}
