...
const UserProfileInfo = ({ user, posts, profileId, setShowEdit }) => {
  ...
  const handleFollow = async () => {
    try {
      ...
      toast.error(
        "Error: " + error.response?.data?.message || "Failed to follow",
      );
  ...
  const handleConnect = async () => {
    try {
      ...
      toast.error(
        "Error: " + error.response?.data?.message || "Failed to connect",
      );
      console.log("Error: ", error);
    } finally {
      setLoading(false);
                ...
                <div className="flex items-center gap-3 mt-4 md:mt-0">
                  ...
                  <div className="relative">
                    <button
                      ...
                      className="size-9 flex items-center justify-center border border-gray-300 hover:bg-gray-50 rounded-lg text-gray-500 hover:text-gray-700 transition cursor-pointer dark:border-zinc-800 dark:hover:bg-zinc-800"
                      title="More actions"
                    >
                    ...
                    {showMoreMenu && (
                      <>
                        ...
                        ></div>
                        <div className="absolute right-0 mt-2 w-36 bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-lg shadow-lg py-1 z-20 animate-fade-in">
                          <button
                            onClick={() => {
                              setShowMoreMenu(false);
                              setShowReport(true);
                            }}
                            className="flex items-center gap-2 w-full px-3 py-2 text-left text-xs font-semibold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 transition cursor-pointer"
                          >
