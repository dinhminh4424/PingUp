    ...
    if (btn.actionType === "lead_form") {
      ...
      fields.forEach((f) => {
        initialAnswers[f.label] =
          f.fieldType === "select" ? f.options?.[0] || "" : "";
      });
  ...
  const handleLeadSubmit = async (e) => {
    ...
    const answersArray = fields.map((f, index) => {
      ...
      return {
        ...
        fileKey:
          f.fieldType === "file" && val instanceof File
            ? `file_${index}`
            : undefined,
      };
    });
  ...
  return (
    <div ref={adRef} className={className} style={{ position: "relative" }}>
      <div className="w-full h-full flex flex-col gap-2.5">
        ...
        {(() => {
          const ctaButtonsList =
            ad.ctaButtons && ad.ctaButtons.length > 0
              ? ad.ctaButtons
              : [
                  {
                    label: "Tìm hiểu thêm",
            ...
            <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden relative text-left flex flex-col max-h-[90vh]">
              ...
              {ad.leadFormConfig?.imageUrl && (
                <div className="w-full aspect-[3/1] overflow-hidden bg-slate-100 border-b border-slate-200/50 shrink-0">
                  <img
                    src={ad.leadFormConfig.imageUrl}
                    className="w-full h-full object-cover"
              ...
              <div
                className="p-6 space-y-4 flex-1 overflow-y-auto no-scrollbar"
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">
                    {ad.leadFormConfig?.description ||
                      "Nhập thông tin bên dưới để kết nối với chúng tôi."}
                  </p>
                </div>
                        ...
                        <div className="flex flex-wrap gap-4 pt-1">
                          {field.options && field.options.length > 0 ? (
                            field.options.map((opt, oIdx) => (
                              <label
                                key={oIdx}
                                className="flex items-center gap-1.5 text-sm text-slate-700 dark:text-zinc-300 cursor-pointer"
                              >
                                <input
                                  type="radio"
                                  required={field.required}
              ...
              #e2e8f0 100%)`,
                                ...
                                <span className="text-xs font-bold font-mono text-indigo-600 pl-4">
                                  {value}
                                </span>
                              </div>
                          ...
                          {answers[field.label] ? (
                            <div className="flex items-center gap-3 p-3 rounded-xl border border-slate-100 bg-slate-50/50 dark:border-zinc-850/50 dark:bg-zinc-950/20">
                              {answers[field.label] instanceof File ? (
                                answers[field.label].type.startsWith(
                                  "image/",
                                ) ? (
                                  <img
                            ...
                            <div className="relative">
                              <input
                                type="file"
                                required={
                                  field.required && !answers[field.label]
                                }
                                onChange={(e) => {
