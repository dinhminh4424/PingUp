  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const res = await getStoryStats(chartPeriod, storyTypeFilter, start, end);
        if (res.success) {
          setData(res);
        } else {
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border/50 pb-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Thống kê Story</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Phân tích số liệu đăng tải, định dạng và lượt xem story của người dùng
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="hover:shadow-md transition-shadow">
          <CardContent className="p-6 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Tổng số Story</p>
              <h3 className="text-3xl font-extrabold text-foreground">{summary.totalStories || 0}</h3>
              <p className="text-xs text-pink-500 font-medium">+{summary.storiesToday || 0} hôm nay</p>
            </div>
            <div className="p-3 bg-pink-100 dark:bg-pink-950/50 rounded-xl text-pink-600 dark:text-pink-400">
              <Tv className="size-6" />
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="hover:shadow-md transition-shadow">
          <CardContent className="p-6 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Tổng lượt xem</p>
              <h3 className="text-3xl font-extrabold text-foreground">{(summary.totalStoryViews || 0).toLocaleString("vi-VN")}</h3>
              <p className="text-xs text-muted-foreground">Người xem tích lũy</p>
            </div>
        ...
        <Card className="hover:shadow-md transition-shadow">
          <CardContent className="p-6 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Tổng lượt thích</p>
              <h3 className="text-3xl font-extrabold text-foreground">{(summary.totalStoryLikes || 0).toLocaleString("vi-VN")}</h3>
              <p className="text-xs text-muted-foreground">Reactions trên story</p>
            </div>
        ...
        <Card className="hover:shadow-md transition-shadow">
          <CardContent className="p-6 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Lượt xem trung bình</p>
              <h3 className="text-3xl font-extrabold text-foreground">
                {summary.totalStories > 0 ? (summary.totalStoryViews / summary.totalStories).toFixed(1) : 0}
              </h3>
      ...
      <Card>
        <CardHeader className="flex flex-col lg:flex-row justify-between lg:items-center gap-4 pb-2 border-b border-border/50">
          <div>
            ...
            <CardDescription>Biến động lượng story đăng tải hàng ngày</CardDescription>
          </div>
        ...
        <CardContent className="h-[280px] pt-6 relative">
          ...
          <div className="h-full w-full">
            {(!chart || chart.length === 0) ? (
              <div className="text-center w-full text-muted-foreground text-xs pt-20">Chưa có dữ liệu thống kê story</div>
            ) : (
              <ChartContainer 
                config={{ 
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Stories thu hút nhiều lượt theo dõi nhất từ bạn bè</CardDescription>
        </CardHeader>
        <CardContent>
          {!leaderboards?.topStories || leaderboards.topStories.length === 0 ? (
            <p className="text-xs text-muted-foreground text-center py-6">Chưa có dữ liệu stories xem nhiều</p>
          ) : (
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody>
                  {leaderboards.topStories.map((story) => (
                    <tr key={story._id} className="border-b border-border/40 hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                      <td className="py-3 flex items-center gap-2">
                        <Avatar className="h-6 w-6">
                          <AvatarImage src={story.userInfo?.profile_picture} />
                          <AvatarFallback className="text-[10px]">{story.userInfo?.full_name?.charAt(0)}</AvatarFallback>
                        </Avatar>
                        <div className="flex flex-col">
                          <span className="font-semibold text-foreground leading-none">{story.userInfo?.full_name}</span>
                          <span className="text-[10px] text-muted-foreground mt-0.5">@{story.userInfo?.username}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-3 text-right text-muted-foreground">
                        {new Date(story.createdAt).toLocaleDateString("vi-VN", { day: "numeric", month: "numeric" })}
                      </td>
                    </tr>
                  ))}
                </tbody>
